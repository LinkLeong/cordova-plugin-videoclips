...
module.exports = function (context) {
    ...
    if (plugins.length > 0 && plugins.indexOf('AkeVideo') === -1) {
        return ;
    }
    ...
    var targetDir  = path.join(projectRoot, "platforms", "android", "src", packageName.replace(/\./g, path.sep), "ake");

 	if (!fs.existsSync(targetDir)) {
		targetDir  = path.join(projectRoot, "platforms", "android", "app", "src", "main", "java", packageName.replace(/\./g, path.sep), "ake");
	} 
    ...
    var targetFiles = ["VideoPlugin.java", "VideoUpActivity.java"];
    ...
    if (['after_plugin_add', 'after_plugin_install'].indexOf(context.hook) === -1) {
        ...
        targetFiles.forEach(function (f) {
            fs.readFile(path.join(context.opts.plugin.dir, 'src', 'android', f), {encoding: 'utf-8'}, function (err, data) {
                ...
                data = data.replace(/^package __PACKAGE_NAME__;/m, 'package ' + packageName + '.ake;');
				data = data.replace(/^import __PACKAGE_NAME__.R;/m, 'import ' + packageName + '.R;');
				data = data.replace(/^import __PACKAGE_NAME__.VideoUpActivity;/m, 'import ' + packageName + '.ake.VideoUpActivity;');
